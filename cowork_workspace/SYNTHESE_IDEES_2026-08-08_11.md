...
### D-7 — Lecture d'écran du smartphone (S25 Ultra)
Distinct du micro/caméra (déjà couvert par le pipeline PWA/`getUserMedia` existant). Lire ce qui s'affiche sur l'écran du téléphone lui-même nécessite un mécanisme différent : l'API Android **MediaProjection** (capture d'écran système), permission dédiée, probablement une petite app compagnon native — `getUserMedia` seul ne capture que caméra/micro, jamais l'écran d'autres applications.
- Devient l'équivalent mobile du système de modes de vision déjà nommé pour le PC (éteint / capture écran / capture document / visionne puis supprime / visionne et sauvegarde), appliqué au téléphone.
- **Garde-fou OTP étendu** : même principe que pour les mails (C-1) — code de vérification affiché pendant une capture = jamais résumé, loggé ni transmis.
- **RT-3** : strictement local.
- **Gating chez moi/dehors** (§4.2) : capture à la demande explicite uniquement, jamais continue — particulièrement strict "dehors".
